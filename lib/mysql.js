...
/**
 * Get a connection from the pool. mysql2 queues getConnection calls with no
 * time limit; with the `acquireTimeout` setting (ms) the caller gets an
 * ER_POOL_ACQUIRE_TIMEOUT error instead. A connection that arrives after the
 * timeout goes straight back to the pool.
 * @param {Function} cb
 */
MySQL.prototype.getPoolConnection = function(cb) {
  const timeout = Number(this.settings.acquireTimeout);
  if (!(timeout > 0)) {
    return this.client.getConnection(cb);
  }
  // mysql2 cannot take a waiting callback out of its queue, so the entry left
  // behind by a timed-out request must not keep the caller alive.
  let callback = cb;
  const timer = setTimeout(function() {
    const done = callback;
    callback = null;
    const err = new Error('Timed out after ' + timeout +
      ' ms waiting for a connection from the pool');
    err.code = 'ER_POOL_ACQUIRE_TIMEOUT';
    done(err);
  }, timeout);
  this.client.getConnection(function(err, connection) {
    if (!callback) {
...
function generateOptions(settings) {
  ...
  const generatorSpecificOptions = [
    ...
    'engine',
    'collation',
    // Read by the connector itself; mysql2 warns about unknown keys.
    'username',
    'createDatabase',
    'acquireTimeout',
  ];
...
MySQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  if (transaction && transaction.connection &&
    ...
    self.getPoolConnection(executeWithConnection);
  }
};
